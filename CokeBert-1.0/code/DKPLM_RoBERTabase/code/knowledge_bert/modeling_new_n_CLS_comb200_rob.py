...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(200, config.intermediate_size, bias=False)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm_ent = BertLayerNorm(200, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class Word_Graph_Attention(nn.Module):
    def __init__(self, k_v_dim, q_dim, graphsage=False, self_att=True):
        ...
        self.K_V_linear_1 = torch.nn.Linear(self.K_V_dim,self.K_V_dim,bias=False)
        init.xavier_uniform_(self.K_V_linear_1.weight)
        self.K_V_linear_2 = torch.nn.Linear(self.K_V_dim,self.K_V_dim,bias=False)
        init.xavier_uniform_(self.K_V_linear_2.weight)
        #init.xavier_uniform_(self.K_weight)
        #nn.init.zeros_(self.K_weight)

        ###
        self.V_linear_1 = torch.nn.Linear(self.K_V_dim,self.K_V_dim,bias=False)
        init.xavier_uniform_(self.V_linear_1.weight)
        self.V_linear_2 = torch.nn.Linear(self.K_V_dim,self.K_V_dim,bias=False)
        init.xavier_uniform_(self.V_linear_2.weight)


        ###
        #self.Q_weight = nn.Parameter(torch.FloatTensor(self.K_V_dim, self.Q_dim))
        ###last_token
        #self.Q_linear_1 = torch.nn.Linear(self.Q_dim*2,self.K_V_dim,bias=True)
        self.Q_linear_1 = torch.nn.Linear(self.Q_dim,self.K_V_dim,bias=True)
        init.xavier_uniform_(self.Q_linear_1.weight)
        #init.xavier_uniform_(self.Q_weight)
        #nn.init.zeros_(self.Q_weight)
        ###last_token
        #self.Q_linear_2 = torch.nn.Linear(self.Q_dim*2,self.K_V_dim,bias=True)
        self.Q_linear_2 = torch.nn.Linear(self.Q_dim,self.K_V_dim,bias=True)
        init.xavier_uniform_(self.Q_linear_2.weight)

        ###
        self.comb = torch.nn.Linear(self.K_V_dim*2, self.K_V_dim, bias=False)
        init.xavier_uniform_(self.comb.weight)
        ...
        self.softmax_3 = nn.Softmax(dim=3)
    ...
    def self_attention(self, Q, K_1, V_1, K_2, V_2, mode=None):
        ...
        #print(K_2.type())
        #print("====")
        #print(self.K_V_linear_2.weight.type())
        #print("====")
        #exit()
        K = self.K_V_linear_2(K_2)
        #K = self.K_V_linear_1(K_2)

        #normalize
        #l2 = torch.norm(K, p=2, dim=4, keepdim=True).detach()
        #l2[l2==0]=float(1e-6)
        #K = K.div(l2)
        #no normalize
        #K = K
        ...
        try:
            sentence_entity_reps = attention.matmul(V_2).squeeze(3)
            #print(sentence_entity_reps.shape) #[16,2,20,100]
            #print(attention.shape)
            #print(V_2.shape)
            #print(sentence_entity_reps.shape)
            #print("=============")
            #torch.Size([8, 1, 20, 1, 20])
            #torch.Size([8, 1, 20, 20, 100])
            #torch.Size([8, 1, 20, 100])
        except:
            print("Line:", 762)
            print(attention.shape)
            print(V_2.shape)
            ...
            exit()
            '''
            if V_2.shape[1] == 0 and attention.shape[1] == 0:
                sentence_entity_reps = torch.zeros(V_1.shape[0],V_1.shape[1],V_1.shape[2],V_1.shape[3]).half().cuda()
                #sent --> [batch, entites, 20, entities, 100]
            '''
        #####
        #####
        ...
        if mode == "candidate_pos":
            ...
            #V_1_pos = V_mask*V_1
            V_1 = V_mask*V_1
            #V_1 = self.comb(torch.cat([V_1_pos, sentence_entity_reps],-1))
            ...
            #V_1_neg = V_mask*V_1
            V_1 = V_mask*V_1
            #V_1 = self.comb(torch.cat([V_1_neg, sentence_entity_reps],-1))
            V_1 = torch.cat([V_1, sentence_entity_reps],-1)
            #V_1 = V_1.mean(2).squeeze(2)
            #return V_1
        else:
            #V_1 = self.comb(torch.cat([V_1, sentence_entity_reps],-1))
            V_1 = torch.cat([V_1, sentence_entity_reps],-1)
        ###
        ...
        try:
            ...
            print(K_l2.shape)
            print(V.shape)
            print("modeling_new.py Error: word_graph_att Class: attention")
            exit()
        ...
        #V_1 = self.V_linear_1(V_1) #1. Original V  2.self.K_V_linear(V) 3.self.V_linear(V)
        try:
    ...
    def forward(self, input_ent, q, k_1, v_1, k_2, v_2, mode):
        ...
        if mode == "entity":
            ...
            if self.graphsage:
                ...
                combined = torch.cat([v, combined], dim=2)
                combined = self.graphsage_linear(combined)
            else:
                combined = combined
            '''

            hidden_states_ent = torch.zeros(input_ent.shape[0], input_ent.shape[1], self.K_V_dim*2).half().cuda()
            ...
            #q = q[:,0,:] #all input: 0, !=0 ###
            #combined = self.self_attention(q, k_1, v_1, k_2, v_2, "candidate_neg") ###
            ...
            if self.graphsage:
                ...
                #combined = self.V_linear(v).squeeze(2)
                #v_1 = v_1[:,:,0,:]
                #combined = self.V_linear_2(v_1).squeeze(2)
                combined = combined #del
            ...
            #combined = self.self_attention(q, k_1, v_1, k_2, v_2, "candidate_pos")
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True, k_1=None, v_1=None, k_2=None, v_2=None, candidate=None, k_cand_1=None, v_cand_1=None,  k_cand_2=None, v_cand_2=None, cand_pos_tensor=None, mode=None):
        ...
        #tacred
        #head = hidden_states[input_ids == 40117]
        #tail = hidden_states[input_ids == 50210]
        #last_token = torch.cat([head, tail], -1)
        ...
        if mode=="pretrain":
            ...
            #last_token = hidden_states[input_ids == 50001]
            last_token = hidden_states[input_ids == 2]
        ...
        if len(input_ent[input_ent!=0]) == 0:
            ...
            hidden_states_ent = self.word_graph_attention(input_ent, last_token, k_1, v_1, k_2, v_2, "entity")
            #print(hidden_states_ent.shape)
        ...
        if mode=="pretrain":
            ...
            k_cand_1 = torch.cat(hidden_states.shape[0]*[k_cand_1])
            v_cand_1 = torch.cat(hidden_states.shape[0]*[v_cand_1])
            cand_pos_tensor = cand_pos_tensor.float().half().unsqueeze(2).unsqueeze(3)
            #k_cand_pos = k_cand*cand_pos_tensor
            #v_cand_pos = v_cand*cand_pos_tensor
            #k_cand_pos_1 = k_cand_1*cand_pos_tensor
            ...
            ###
            #candidate_pos = self.word_graph_attention(candidate, hidden_states, k_cand_1, v_cand_pos_1, k_cand_2, v_cand_2, "candidate_pos") #
            candidate_pos = self.word_graph_attention(candidate, last_token, k_cand_1, v_cand_pos_1, k_cand_2, v_cand_2, "candidate_pos") #
            ###
            ...
            #print("===")
            #print(hidden_states.shape)
            #print(v_cand_neg.shape)
            #print(v_cand_1.shape)
            #candidate_pos = self.word_graph_attention(candidate, hidden_states, None, v_cand_pos_1, None, v_cand_2, "candidate_neg") #
            ###
            #candidate_neg = self.word_graph_attention(candidate, hidden_states, k_cand_1, v_cand_neg_1, k_cand_2, v_cand_2, "candidate_neg") #
            candidate_neg = self.word_graph_attention(candidate, last_token, k_cand_1, v_cand_neg_1, k_cand_2, v_cand_2, "candidate_neg") #
            ###
            #candidate_neg = self.word_graph_attention(candidate, hidden_states, None, v_cand_neg, None, None, "candidate_neg") #
            #print(candidate_neg.shape)

            candidate = candidate_pos + candidate_neg
            #print("---")
            #print(candidate_pos.shape)
            #print(candidate_neg.shape)
            #print(candidate.shape)
            #exit()
        ...
        #return encoded_layers, pooled_output, hidden_states
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        input_ent=None, ent_mask=None, next_sentence_label=None, candidate=None, ent_labels=None, k_1=None, v_1=None, k_2=None, v_2=None, k_cand_1=None, v_cand_1=None, k_cand_2=None, v_cand_2=None, cand_pos_tensor=None):
        ...
        sequence_output, pooled_output, candidate_emb = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False, k_1=k_1, v_1=v_1, k_2=k_2, v_2=v_2, candidate=candidate, k_cand_1=k_cand_1, v_cand_1=v_cand_1, k_cand_2=k_cand_2, v_cand_2=v_cand_2, cand_pos_tensor=cand_pos_tensor, mode="pretrain")
...
class BertForEntityTyping(PreTrainedBertModel):
    def __init__(self, config, num_labels=2, args=None):
        ...
        #self.typing = nn.Linear(config.hidden_size*2, num_labels, False)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, labels=None, k_1=None, v_1=None, k_2=None, v_2=None):
        seq_out, pooled_output, _ = self.bert(input_ids, token_type_ids, attention_mask, input_ent.long(), ent_mask, output_all_encoded_layers=False, k_1=k_1, v_1=v_1, k_2=k_2, v_2=v_2, candidate=None, k_cand_1=None, v_cand_1=None, k_cand_2=None, v_cand_2=None, cand_pos_tensor=None, mode="typing")


        ###
        #pooled_output = seq_out[input_ids == 40117]

        if self.args.data_token == "open":
            pooled_output = seq_out[input_ids == 50001]
        elif self.args.data_token == "figer":
            pooled_output = seq_out[input_ids == 50001]
...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, args=None):
        ...
        self.bert = BertModel(config,args)
        ###
        self.dropout_NSP = nn.Dropout(config.hidden_dropout_prob)
        self.classifier_NSP = nn.Linear(config.hidden_size, num_labels)
        ###
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, labels=None, k_1=None, v_1=None, k_2=None, v_2=None):
        ...
        seq_out, pooled_output, hidden_states_6th_original = self.bert(input_ids, token_type_ids, attention_mask, input_ent.long(), ent_mask, output_all_encoded_layers=False, k_1=k_1, v_1=v_1, k_2=k_2, v_2=v_2, candidate=None, k_cand_1=None, v_cand_1=None, k_cand_2=None, v_cand_2=None, cand_pos_tensor=None, mode="classification")


        ###
        #NSP
        '''
        pooled_output = self.dropout_NSP(pooled_output)
        logits_NSP = self.classifier_NSP(pooled_output)
        '''
        ###
        if self.args.data_token == "tacred":
            head = seq_out[input_ids == 40117]
            tail = seq_out[input_ids == 50210]
        elif self.args.data_token == "fewrel":
        ...
        try:
            ...
            print("WRONG!!!")
            print("modeling_new_n_CLS_comb200_rob")
        ...
        if labels is not None:
            ...
            return loss
            '''
            loss_fct_NSP = CrossEntropyLoss()
            loss_NSP = loss_fct_NSP(logits_NSP.view(-1, self.num_labels), labels.view(-1))
            return loss+loss_NSP
            #return loss_NSP
            '''
            ###
        else:
