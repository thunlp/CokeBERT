...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, args=None):
        ...
        self.apply(self.init_bert_weights)
        self.comb = nn.Linear(num_labels*2, num_labels)
        self.c = nn.Parameter(torch.Tensor(2,1,1))
        softmax_0 = nn.Softmax(dim=0)
        #self.comb = nn.weight(2, 1)
    ...
    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, labels=None, k=None, v=None, logits_transE=None):
        ...
        #logits = self.comb( torch.cat([logits, logits_transE],dim=1) )
        #logits = logits+logits_transE

        #print(logits.shape)
        #print(logits_transE.shape)
        #print("---")
        logits = torch.stack([logits,logits_transE])
        #print(logits.shape)
        logits = (self.c*logits).sum(dim=0)
        #print(logits.shape)
        #print("====")
