...
class BertEncoder(nn.Module):
    def __init__(self, config, args=None):
        ...
        for t in config.layer_types:
            ...
            if t == "norm":
                layers_last6_att.append(copy.deepcopy(layer))
                layers_last6_transE.append(copy.deepcopy(layer))
            if t == "mix":
                layers_last6_att.append(copy.deepcopy(layer_mix))
                layers_last6_transE.append(copy.deepcopy(layer_mix))
        #for _ in range(config.num_hidden_layers-len(layers)):
        for _ in range(config.num_hidden_layers-len(layers+layers_last6_att)):
            print("Wrong!!!")
            exit()
            layers.append(copy.deepcopy(layer_simple))

        self.layer = nn.ModuleList(layers)
        self.layer_att = nn.ModuleList(layers_last6_att)
        self.layer_transE = nn.ModuleList(layers_last6_transE)
        ####
    ...
    def forward(self, hidden_states, all_encoder_layers, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask, i_th_layer=0, output_all_encoded_layers=True, mode=None):
        ...
        if mode=="encoder1":
        ...
        elif mode=="encoder2_att":

            #print("====")
            for i_th_layer,layer_module in enumerate(self.layer_att):
        ...
        elif mode=="encoder2_transE":
            for i_th_layer,layer_module in enumerate(self.layer_transE):
                #print(i_th_layer,self.layer_types[5+i_th_layer])
                hidden_states, hidden_states_ent = layer_module(hidden_states, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask)
            ...
            print("Have no mode {]".format(mode))
            print("!!WRONG!!")
            exit()
...
class Word_Graph_Attention(nn.Module):
    ...
    def self_attention(self, Q, K, V):
        ...
        ###########
        ####Add####
        #scale = torch.max(attention,2).values.unsqueeze(2)
        #attention[attention!=scale] = 0
        #attention[attention==scale] = 1
        ###########
        ###########
    ...
    def forward(self, input_ent, q, k, v, mode):
        ...
        if mode=="candidate_neg":
            #v_att = v[:,:,0,:]
            v_ = v[:,:,0,:]
            v_att = self.V_linear(v_).squeeze(2)
            return v_att, v_ #didn't
            #return v_att, v_att #did
        ...
        if mode == "entity":
            ...
            hidden_states_ent_att = torch.zeros(input_ent.shape[0], input_ent.shape[1], self.K_V_dim).half().cuda()
            hidden_states_ent_transE = torch.zeros(input_ent.shape[0], input_ent.shape[1], self.K_V_dim).half().cuda()
            ...
            #####
            '''
            #v = self.V_linear(v) #did
            for batch in range(input_ent.shape[0]):
                for i,index in enumerate(ent_pos_s[ent_pos_s[:,0]==batch]):
                    hidden_states_ent_att[batch][int(index[1])] = combined[batch][i]
                    hidden_states_ent_transE[batch][int(index[1])] = v[batch][i][0] #didn't
                    #hidden_states_ent_transE[batch][int(index[1])] = v[batch][i][0] #did

        elif mode == "candidate_pos":
            hidden_states_ent_att = combined
            hidden_states_ent_transE = v[:,:,0,:] #didn't
            #hidden_states_ent_transE = self.V_linear(v[:,:,0,:]) #did
        ...
        return hidden_states_ent_att, hidden_states_ent_transE
...
class BertModel(PreTrainedBertModel):
    ...
    def __init__(self, config, args=None):
        ...
        self.pooler_att = BertPooler(config)
        self.pooler_transE = BertPooler(config)
        self.apply(self.init_bert_weights)
        self.word_graph_attention = Word_Graph_Attention(args.K_V_dim,args.Q_dim,args.graphsage,args.self_att)


    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True, k=None, v=None, candidate=None, k_cand=None, v_cand=None, cand_pos_tensor=None, task_mode=None):
        ...
        hidden_states_ent_att, hidden_states_ent_transE = self.word_graph_attention(input_ent, hidden_states, k, v, "entity")
        #hidden_states_ent = self.word_graph_attention(input_ent, hidden_states[:,0,:], k, v, "entity")

        #all_encoder_layers_att = all_encoder_layers.copy()
        #all_encoder_layers_transE = all_encoder_layers.copy()

        #att
        _, encoded_layers_att, _ = self.encoder(hidden_states, all_encoder_layers, extended_attention_mask, hidden_states_ent_att, extended_ent_mask, ent_mask, i_th_layer=i_th_layer, output_all_encoded_layers=output_all_encoded_layers, mode="encoder2_att")

        #transE
        _, encoded_layers_transE, _ = self.encoder(hidden_states, all_encoder_layers, extended_attention_mask, hidden_states_ent_transE, extended_ent_mask, ent_mask, i_th_layer=i_th_layer, output_all_encoded_layers=output_all_encoded_layers, mode="encoder2_transE")

        if task_mode=="pretrain":
            ...
            candidate_pos_att, candidate_pos_transE = self.word_graph_attention(candidate, hidden_states, k_cand_pos, v_cand_pos, "candidate_pos") #

            cand_pos_tensor = ((cand_pos_tensor-1)*(-1))
            v_cand_neg = v_cand*cand_pos_tensor
            candidate_neg_att, candidate_neg_transE = self.word_graph_attention(candidate, hidden_states, None, v_cand, "candidate_neg") #

            candidate_att = candidate_pos_att + candidate_neg_att
            candidate_transE = candidate_pos_transE + candidate_neg_att
        else:
            candidate_att = None
            candidate_transE = None

        #att
        sequence_output = encoded_layers_att[-1]
        pooled_output_att = self.pooler_att(sequence_output)
        if not output_all_encoded_layers:
            encoded_layers_att = encoded_layers_att[-1]

        #transE
        sequence_output = encoded_layers_transE[-1]
        pooled_output_transE = self.pooler_transE(sequence_output)
        if not output_all_encoded_layers:
            encoded_layers_transE = encoded_layers_transE[-1]

        return encoded_layers_att, pooled_output_att, candidate_att, encoded_layers_transE, pooled_output_transE, candidate_transE
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def __init__(self, config, args):
        ...
        self.cls_TransE = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight) ###
        self.apply(self.init_bert_weights)
        self.args = args
        #self.word_graph_attention = Word_Graph_Attention(args.K_V_dim,args.Q_dim,args.graphsage,args.self_att)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        sequence_output, pooled_output, candidate_emb, sequence_output_transE, pooled_output_transE, candidate_emb_transE = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False, k=k, v=v, candidate=candidate, k_cand=k_cand, v_cand=v_cand, cand_pos_tensor=cand_pos_tensor, task_mode="pretrain")

        #Att
        prediction_scores, seq_relationship_score, prediction_scores_ent = self.cls(sequence_output, pooled_output, candidate_emb)
        #TransE
        prediction_scores_transE, seq_relationship_score_transE, prediction_scores_ent_transE = self.cls_TransE(sequence_output_transE, pooled_output_transE, candidate_emb_transE)
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            #Trans
            masked_lm_loss_transE = loss_fct(prediction_scores_transE.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))
            ...
            #TransE
            next_sentence_loss_transE = loss_fct(seq_relationship_score_transE.view(-1, 2), next_sentence_label.view(-1))


            #Att
            ent_ae_loss = loss_fct(prediction_scores_ent.view(-1, candidate.shape[1]), ent_labels.view(-1))
            #TransE
            ent_ae_loss_transE = loss_fct(prediction_scores_ent_transE.view(-1, candidate.shape[1]), ent_labels.view(-1))


            #total_loss = masked_lm_loss + next_sentence_loss + ent_ae_loss
            total_loss = masked_lm_loss + next_sentence_loss + ent_ae_loss +  masked_lm_loss_transE + next_sentence_loss_transE + ent_ae_loss_transE

            #original_loss = masked_lm_loss + next_sentence_loss
            original_loss = masked_lm_loss + next_sentence_loss + masked_lm_loss_transE + next_sentence_loss_transE
            return total_loss, original_loss
        else:
            #return prediction_scores, seq_relationship_score, prediction_scores_ent
            return prediction_scores+prediction_scores_transE, seq_relationship_score+seq_relationship_score_transE, prediction_scores_ent+prediction_scores_transE
...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, args=None):
        ...
        self.classifier_TransE = nn.Linear(config.hidden_size, num_labels) ###
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, labels=None, k=None, v=None):
        ...
        #_, pooled_output, hidden_states_6th_original = self.bert(input_ids, token_type_ids, attention_mask, input_ent.long(), ent_mask, output_all_encoded_layers=False, k=k, v=v, candidate=None, k_cand=None, v_cand=None, cand_pos_tensor=None, mode="classfication")


        #####
        sequence_output, pooled_output, candidate_emb, sequence_output_transE, pooled_output_transE, candidate_emb_transE = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False, k=k, v=v, candidate=None, k_cand=None, v_cand=None, cand_pos_tensor=None, task_mode="classification")

        #Att
        #prediction_scores, seq_relationship_score, prediction_scores_ent = self.cls(sequence_output, pooled_output, candidate_emb)
        #TransE
        #prediction_scores_transE, seq_relationship_score_transE, prediction_scores_ent_transE = self.cls(sequence_output_transE, pooled_output_transE, candidate_emb_transE)
        #####

        #_, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False)

        pooled_output = self.dropout(pooled_output)
        pooled_output_transE = self.dropout(pooled_output_transE)
        logits = self.classifier(pooled_output)
        #logits_transE = self.classifier(pooled_output_transE)
        logits_transE = self.classifier_TransE(pooled_output_transE) ###

        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            loss_transE = loss_fct(logits_transE.view(-1, self.num_labels), labels.view(-1))
            return loss+loss_transE
            #return loss
        else:
            return logits+logits_transE
            #return logits
