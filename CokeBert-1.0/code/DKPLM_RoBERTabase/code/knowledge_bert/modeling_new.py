...
class Word_Graph_Attention(nn.Module):
    def __init__(self, k_v_dim, q_dim, graphsage=False, self_att=True):
        ...
        ###
        #self.Q_linear = torch.nn.Linear(self.Q_dim*2,self.K_V_dim,bias=True)
        self.Q_linear = torch.nn.Linear(self.Q_dim,self.K_V_dim,bias=True)
        ###
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True, k=None, v=None, candidate=None, k_cand=None, v_cand=None, cand_pos_tensor=None, mode=None):
        ...
        print("1277:",hidden_states.shape)
        exit()
        '''
        ...
        #hidden_states_ent = self.word_graph_attention(input_ent, torch.cat([head,tail],-1), k, v, "entity")
        ###
        ...
        if mode=="pretrain":
            ...
            ###
            #candidate_pos = self.word_graph_attention(candidate, torch.cat([head,tail],-1), k_cand_pos, v_cand_pos, "candidate_pos") #
            ...
            v_cand_neg = v_cand*cand_pos_tensor
            ###
            #candidate_pos = self.word_graph_attention(candidate, torch.cat([head,tail],-1), None, v_cand_pos, "candidate_neg") #
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            '''
            print(seq_relationship_score.shape)
            print(seq_relationship_score.view(-1,2).shape)
            print("---")
            print(next_sentence_label.shape)
            print(next_sentence_label.view(-1).shape)
            exit()
            '''
