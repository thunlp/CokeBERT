...
def main():
    ...
    model, _ = BertForSequenceClassification.from_pretrained(args.ernie_model,
              cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(args.local_rank),
              num_labels = num_labels, embed = embed.half().to(device), adj_list = adj_list)
    '''

    #print(model)
    #print(model.parameters()) #will list not being initlized paras
    #print(model['bert.encoder.embed_ent.weight'])
    #print(model.BertEncoder)
    #print(model.bert.encoder.embed_ent.weight)
    #print(type(model.bert.encoder))
    #exit()
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                #loss = model(input_ids, segment_ids, input_mask, input_ent, ent_mask, label_ids, args.fp16)
                ##################
                ...
                #####------------
                #print(input_ent)
                #print(input_ent[input_ent>-1])
                input_ent = input_ent+1


                ###test
                '''
                input_ent = torch.zeros(2,256).long()
                input_ent[0,0] = 9
                input_ent[0,1] = 2
                input_ent[1,0] = 1
                input_ent[1,1] = 3
                ...
                ##################
                ##################
                #Test: load_ent_emb_dynamic/static, load_(batch)_k_v_queryE/R, input==>self assign nodes
                ###
                #input_ent = input_ent%9
                ###

                #start_time = time.time()
                #k,v = load_k_v_queryE(input_ent)
                ...
                loss=0
                if args.fp16:
                    #input_ent = embed(input_ent.to(device))
                    ...
                    #loss = model(input_ids, segment_ids, input_mask, input_ent.float(), ent_mask, label_ids, device, input_ent_emb)


                    #use parallel --> pass variable must be diviable
                    #loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, input_ent=input_ent.half(), ent_mask=ent_mask, labels=label_ids, subgraph=subgraph, subgraph_id_map_rep=subgraph_id_map_rep)
                    #loss = model(input_ids, segment_ids, input_mask, input_ent.half(), ent_mask, label_ids)
                else:
                    #loss = model(input_ids, segment_ids, input_mask, input_ent, ent_mask, label_ids, subgraph, subgraph_id_map_rep)
