...
from knowledge_bert.modeling_new_n_CLS_comb200 import BertForPreTraining
...
def load_k_v_queryR(input_ent,device):
        ...
        return k,v
        ###
        #return input_ent_neighbor_emb, input_ent_r_emb, input_ent_outORin_emb
...
def main():
    ...
    if args.do_train:
        ...
        def collate_fn(x):
            ...
            #print(ent_emb)
            #print(ent_emb.shape)
            #k,v = load_k_v_queryR(ent_emb)
            #k_cand,v_cand = load_k_v_queryR(ent_candidate)
            #exit()
            ##########################
            ##########################
            ...
            #return x[:,:args.max_seq_length], x[:,args.max_seq_length:2*args.max_seq_length], x[:,2*args.max_seq_length:3*args.max_seq_length], x[:,3*args.max_seq_length:4*args.max_seq_length], ent_emb, mask, x[:,6*args.max_seq_length:], ent_candidate, ent_labels, k, v, k_cand, v_cand
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_iterator.next_epoch_itr(), desc="Iteration")):
                ...
                #input_ids, input_mask, segment_ids, masked_lm_labels, input_ent, ent_mask, next_sentence_label, ent_candidate, ent_labels, k, v, k_cand, v_cand = batch
                ...
                #k_cand, v_cand = load_k_v_queryE(ent_candidate)
                ...
                if args.fp16:
                    ...
                    #print("model:{}".format(end_time_3-end_time_2))
                    #loss, original_loss = model(input_ids, segment_ids, input_mask, masked_lm_labels, input_ent, ent_mask, next_sentence_label, ent_candidate.half(), ent_labels, n_gpu)
                else:
