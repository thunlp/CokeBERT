...
class Word_Graph_Attention(nn.Module):
    ...
    def forward(self, input_ent, q, k, v, mode, ent_n):

        q = q[:,0,:] #all input: 0, !=0
        return self.self_attention(q,k,v,"entity",ent_n)

        '''
        if mode == "entity":
            q = q[:,0,:] #all input: 0, !=0
            neigh_feats = self.self_attention(q, k, v, mode)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True, k=None, v=None, candidate=None, k_cand=None, v_cand=None, cand_pos_tensor=None, mode=None, ent_n=None):
        ...
        if len(input_ent[input_ent!=0]) == 0:
            ...
            ids = self.word_graph_attention(input_ent, hidden_states, k, v, "entity", ent_n)
