...
from knowledge_bert.modeling_new_n_CLS_comb200 import BertForEntityTyping
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer_label, tokenizer, threshold):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        ex_text_a = ex_text_a[:h[1]] + "∞ " + ex_text_a[h[1]:h[2]] + " º" + ex_text_a[h[2]:]
        begin, end = h[1:3]
        h[1] += 2
        h[2] += 2
    ...
    for x, mark in file_mark:
        ...
        for input_ids, input_mask, segment_ids, input_ent, ent_mask, labels in eval_dataloader:
            ...
            with torch.no_grad():
                ...
                tmp_eval_loss = model(input_ids, segment_ids, input_mask, input_ent, ent_mask, labels.half(), k_1.half(), v_1.half(), k_2.half(), v_2.half())
                #exit()
